...
if ($selected_user_id > 0) {
    ...
    if ($selected_user) {
        // Get all messages in this conversation with claim information
        $stmt = $pdo->prepare("
            SELECT m.*, 
                   u.name as sender_name,
                   c.id as claim_id_ref,
        ...
        // Mark messages as read - DO NOT mark yet until they actually open the conversation
        // This is handled by selecting a conversation
        $stmt = $pdo->prepare("
            UPDATE messages 
            SET is_read = 1 
...
<style>
    ...
    .chat-header {
        padding: 16px;
        border-bottom: 1px solid #e0e0e0;
        background: linear-gradient(135deg, #ffc107 0%, #ed1c24 100%);
        color: white;
        display: flex;
        justify-content: space-between;
        align-items: center;
...
<div class="container-fluid py-3" style="height: 90%; padding: 0 !important; max-width: 1350px; margin: 0 auto;">
                    ...
                    <?php if ($selected_user): ?>
                        ...
                        <div class="chat-header">
                            <div style="display: flex; gap: 10px; align-items: center; width: 100%; justify-content: space-between;">
                                <button class="btn btn-sm btn-light d-lg-none" style="border: none; background: rgba(255,255,255,0.2); color: white;" onclick="toggleConversations()">
                            ...
                            <?php if (!empty($messages)): ?>
                                <?php foreach ($messages as $msg): ?>
                                    <div class="message-bubble <?php echo $msg['is_sent'] ? 'sent' : 'received'; ?>">
                                        <?php if (!empty($msg['claim_id_ref']) && !empty($msg['item_title'])): ?>
                                            <div class="claim-context" style="background-color: <?php echo $msg['is_sent'] ? '#e3f2fd' : '#fff9e6'; ?>; padding: 8px 12px; border-radius: 6px; margin-bottom: 8px; font-size: 12px; border-left: 3px solid <?php echo $msg['is_sent'] ? '#2196F3' : '#FFC107'; ?>;">
                                                <strong><i class="bi bi-reply"></i> Re: <?php echo htmlspecialchars($msg['item_title']); ?></strong>
                                                <br><small style="color: #666;">Claim #<?php echo $msg['claim_id_ref']; ?></small>
                                            </div>
                                        <?php endif; ?>
...
<script>
    ...
    function selectConversation(userId) {
        window.location.href = '<?php echo BASE_URL; ?>/messages.php?user_id=' + userId;
    }
    ...
    // Toggle conversations panel on mobile
    ...
    // Close conversations panel when selecting a conversation on mobile
    const conversationItems = document.querySelectorAll('.conversation-item');
    conversationItems.forEach(item => {
        item.addEventListener('click', function() {
            // Close panel on mobile/tablet after selection
    ...
    document.addEventListener('click', function(event) {
        const panel = document.querySelector('.conversations-panel');
        const toggleBtn = event.target.closest('.d-lg-none');
        
        if (window.innerWidth < 992 && panel && panel.classList.contains('show')) {
            // If clicked outside the panel and it's not a button/interactive element inside panel
            if (!event.target.closest('.conversations-panel') && !toggleBtn) {
                // Check if we're clicking on a conversation item (handled above)
